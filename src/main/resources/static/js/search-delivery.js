...
//style the message box
        ...
        function SaveData(msg,companyid,id) {
            ...
            var outDay = msg[7];
            var saveDay = msg[8];
            var wareAbility = msg[9];
            var deliveryDay = msg[10];
            var range = msg[11];
            var radius = msg[12];
            var deliveryNum = msg[13];
            var carNum = msg[14];
            var deliveryDayReal = msg[15];
            var status;
            if(msg[16] == "未选择"){
                status = 0;
            }
            ...
            $.ajax({
                ...
                data: {companyId:companyId,name:name,province:province,city:city,county:county,socialId:socialId,location:location,principal:principal,phone:phone,
                    inDay:inDay,outDay:outDay,saveDay:saveDay,deliveryId:deliveryId,wareAbility:wareAbility,deliveryDay:deliveryDay,range:range,radius:radius,deliveryNum:deliveryNum,carNum:carNum,deliveryDayReal:deliveryDayReal,state:status},
                async:false,
                success: function(data){
                    // initnewAttributeTable();
                },
