...
jQuery(function($) {
    ...
    $("#search_machine").click(function(){

        var time1=$("#time1").val();
        var time2=$("#time2").val();
        $.ajax({
            type: "POST",
            url: "toMachineShowByDate",
...
//style the message box
        ...
        $("#dynamic-table tbody").unbind('click').on("click", ".edit-btn", function () {
            ...
            $.each(tds, function (i, val) {
                var jqob = $(val);
                var txt = jqob.text();
                if (i > 2 && i< tds.length-2) {
                    var put = $("<input type='text' class='form-control' style='width:100%'>");
                    put.val(txt);
                    jqob.html(put);
                }
                if(i == tds.length-2){
                    var put = $("<select class='form-control' style='width:100%'><option value ='未选择'>未选择</option><option value ='生产中'>生产中</option><option value ='停产'>停产</option></select>");
                    put.val(txt);
                    jqob.html(put);
                }
            });
        ...
        $("#dynamic-table tbody").on("click", ".save-btn", function () {
            var tds = $(this).parents("tr").children();
            var companyid = tds.eq(1).text();
            var id = tds.eq(2).text();
            var sTds = tds.length;
            var lisval = [];
            for (var i = 3; i < sTds; i++) {
                if (i < sTds - 2)
                    lisval.push(tds.eq(i).children("input").val());
                if (i == sTds - 2)
                    lisval.push(tds.eq(i).children("select").val());
            }
            ...
            $.each(tds, function (i, val) {
                ...
                if (!jqob.has('i').length) {
                    var txt = (jqob.has('input').length)?jqob.children("input").val():jqob.children("select").val();
                    jqob.html(txt);
                }
            });
            $(this).toggleClass("edit-btn fa-pencil");
            $(this).toggleClass("save-btn fa-save");
            //保存数据
            SaveData(lisval,companyid,id);
        });

        //设置保存信息
        function SaveData(msg,companyid,id) {
            ...
            var paddyDay = msg[10];
            var oilDay = msg[11];
            var oilConciseDay = msg[12];
            var oilSubpDay = msg[13];
            var elseDay = msg[14];
            var wheatDayReal = msg[15];
            var paddyDayReal = msg[16];
            var oilDayReal = msg[17];
            var oilConciseDayReal = msg[18];
            var oilSubpDayReal = msg[19];
            var elseDayReal = msg[20];
            var status;
            if(msg[21] == "未选择"){
                status = 0;
            }
            ...
            $.ajax({
                ...
                data: {companyId:companyId,name:name,province:province,city:city,county:county,socialId:socialId,location:location,principal:principal,phone:phone,
                    inDay:inDay,outDay:outDay,saveDay:saveDay,machineId:machineId,wheatDay:wheatDay,paddyDay:paddyDay,oilDay:oilDay,oilConciseDay:oilConciseDay,oilSubpDay:oilSubpDay,elseDay:elseDay,
                    wheatDayReal:wheatDayReal,paddyDayReal:paddyDayReal,oilDayReal:oilDayReal,oilConciseDayReal:oilConciseDayReal,oilSubpDayReal:oilSubpDayReal,elseDayReal:elseDayReal,state:status},
                async:false,
                success: function(data){
                    // initnewAttributeTable();
                },
